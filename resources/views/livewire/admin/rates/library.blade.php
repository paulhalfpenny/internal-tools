<div x-data="{ showForm: false }">
    ...
    <div x-show="showForm" x-cloak class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        <div class="flex items-center justify-between mb-3">
            <h2 class="text-sm font-medium text-gray-700">Add rate</h2>
        ...
        <div class="flex gap-3 items-end">
            <div class="flex-1">
                ...
                <input wire:model="name" type="text" placeholder="e.g. Standard Dev" class="w-full border border-gray-300 rounded text-sm px-3 py-2">
                @error('name')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="w-40">
                <label class="block text-xs font-medium text-gray-600 mb-1">Hourly rate (£) <span class="text-red-500">*</span></label>
                <input wire:model="hourlyRate" type="number" step="0.01" min="0" class="w-full border border-gray-300 rounded text-sm px-3 py-2">
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right font-medium text-gray-600">Hourly rate</th>
                ...
                @forelse($rates as $rate)
                    @if($editingId === $rate->id)
                        <tr class="bg-blue-50">
                            ...
                            <td class="px-4 py-2">
                                <input wire:model="editHourlyRate" type="number" step="0.01" min="0" class="w-32 border border-gray-300 rounded text-sm px-3 py-2 text-right">
                    ...
                    @else
                        <tr class="{{ $rate->is_archived ? 'opacity-50' : '' }}">
                            <td class="px-4 py-3 font-medium">{{ $rate->name }}</td>
                            <td class="px-4 py-3 text-right tabular-nums">£{{ number_format((float) $rate->hourly_rate, 2) }}</td>
                            <td class="px-4 py-3">
                                @if($rate->is_archived)
                            ...
                            <td class="px-4 py-3 text-right space-x-3">
                                <button wire:click="edit({{ $rate->id }})" class="text-sm text-blue-600 hover:underline">Edit</button>
                                <button wire:click="toggleArchive({{ $rate->id }})" class="text-sm text-gray-400 hover:text-gray-600 hover:underline">
                                    {{ $rate->is_archived ? 'Unarchive' : 'Archive' }}
                                </button>
                            </td>
                        </tr>
                    @endif
                @empty
                    <tr><td colspan="4" class="px-4 py-8 text-center text-gray-400 text-sm">No rates yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
